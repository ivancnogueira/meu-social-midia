...
Na etapa 4, configure primeiro as credenciais localmente: guie a criação do app Meta, repositório separado do GitHub Pages (modo local) ou domínio HTTPS (servidor), explique cada campo e peça que o usuário preencha os tokens somente no `.env`. Não exponha credenciais no chat. Antes de gerar arte, valide `npm run diagnosticar -- --rede` ou `npm run validar:integracoes`: campo preenchido não vale como conexão funcional. A validação consulta Meta, repositório e branch do Pages e registra somente um resultado sanitizado em `runtime/`.
...
Depois da aprovação visual, obtenha autorização específica para upload público e envie PNG, preview HTML e vitrine ao GitHub Pages. Execute `npm run pages:validar -- CAMINHO_PUBLICACAO` e só entregue o link quando o HTTPS responder; a URL pública do PNG é a mídia que a Meta busca, sem uma “hospedagem da Meta” adicional. Depois disso gere o job, mostre o ID, resuma a versão imutável e peça ao usuário que responda exatamente `APROVAR ID-DO-JOB`. Ao receber essa confirmação no chat, registre a aprovação local auditável e publique pela API oficial. Aprovação de preview, “pode seguir” genérico ou autorização dada antes de o job existir não autoriza publicação.
